...
def parse_arguments() -> Tuple[Optional[str], Optional[str], Optional[int], Optional[List[str]]]:
    parser = argparse.ArgumentParser(description='XML2YAMl CLI program. Converts OJS XML to PhiMiSci/pandoc YAML.')
...
def main(xml_filepath: Optional[str], year: Optional[str], volume: Optional[str], orcid: Optional[List[str]]) -> None:
    ...
    if publication_data.find("{http://pkp.sfu.ca}abstract") is not None:
        ...
        assert abstract_element is not None, "Abstract element not found"
    ...
    if (publication_data.find(".//{http://pkp.sfu.ca}volume") is not None) and (volume is None):
        ...
        assert volume_element is not None, "Volume element not found"
    ...
    if publication_data.find(".//{http://pkp.sfu.ca}keywords") is not None:
        ####### parse keyword-string for Latex/PDF
        keyword_node = publication_data.find(".//{http://pkp.sfu.ca}keywords")
        assert isinstance(keyword_node, lxml.etree._Element), "Keywords node not found"
    ...
    if year is not None:
        ...
        if re.match(r"\d{4}", year):
            ...
            # TODO: maybe just check publication dear with current year
...
if __name__ == "__main__":
    # Parse arguments
    xml_file_path, year, volume, orcid = parse_arguments()
    # Run main program
    main(xml_file_path, year, volume, orcid)

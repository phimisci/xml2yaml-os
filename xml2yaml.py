...
def parse_arguments():
    ...
    # Additional year field might be necessary if the year is not present in 
    # the XML file
    parser.add_argument("-y", "--year", type=str, help="Year of publication")
    parser.add_argument("-v", "--volume", type=str, help="Volume number")
    # Sometimes, the ORCID is not present in the XML file. In this case, 
    # the ORCID can be passed as a command line argument
    # Example: --orcid Starke=0000-0001-1111-1111 Jurczyk=0000-0002-5943-2305
    parser.add_argument("-o", "--orcid", type=str, nargs="+",
                        help="""ORCID key-value pairs of authors (separated via 
                        blank space when multiple authors): 
                        --orcid <AUTHOR_LASTNAME>=<ORCID> 
...
def main(xml_filepath: Optional[str], year: Optional[str], volume: Optional[str], 
    ...
    '''Main program logic to convert XML2YAML.
        ...
        ----------
            xml_filepath: Optional[str]
                The path to the XML file (cannot be None actually because of 
            ...
            orcid: Optional[str]
                String with key-value pairs of <AUTHOR_LASTNAME> and <ORCID>. 
                Example: --orcid Starke=0000-0001-1111-1111 
                Jurczyk=0000-0002-5943-2305 (this string needs to be parsed later on.
            special_issue: Optional[str]
                A string with information about special issue. Used in articles 
                that are part of a special issue. 
    ...
    if publication_data.find("{http://pkp.sfu.ca}title") is not None:
        ...
        # 2. Title with main title and subtitle("This is an article: It is 
        #    interesting" or "Is this an article? If so, it is interesting")
        # Split on either : or ?
        title_list = [title.strip() for title in re.split(
            r"(:|\?)", title.strip(), maxsplit=1) if title != ""]
        # the title seems to be missing or is incorrect
        if len(title_list) > 3 and len(title_list) == 0:
            logging.warning(
                """There seems to be an issue with the title parsing. Please 
                   check manually.""")
            data_dict["title"] = SingleQuotedString(title)
            data_dict["title-hdr"] = SingleQuotedString(title)
            if subtitle:
                data_dict["title-meta"] = SingleQuotedString(
                    f"{title}: {subtitle}.")
            else:
            ...
            if len(title_list) == 3:
                data_dict["title"] = SingleQuotedString(
                    title_list[0]) if title_list[1] == ":" else SingleQuotedString(title_list[0]+title_list[1])
                data_dict["subtitle"] = SingleQuotedString(title_list[2])
                data_dict["title-hdr"] = SingleQuotedString(
                    title_list[0]) if title_list[1] == ":" else SingleQuotedString(title_list[0]+title_list[1])
                data_dict["title-meta"] = SingleQuotedString(
                    title_list[0] + f"{title_list[1]} " + title_list[2] + ".")
        ...
        logging.warning(
            "No title field found in XML. Replacing with NO_TITLE_FIELD_FOUND_IN_XML.")
        data_dict["title"] = SingleQuotedString("NO_TITLE_FIELD_FOUND_IN_XML")
        data_dict["title-hdr"] = SingleQuotedString(
            "NO_TITLE_FIELD_FOUND_IN_XML")
        data_dict["title-meta"] = SingleQuotedString(
            "NO_TITLE_FIELD_FOUND_IN_XML"+".")
    ...
    if publication_data.find("{http://pkp.sfu.ca}abstract") is not None:
        ...
        logging.warning(
            "No abstract field found in XML. Replacing with NO_ABSTRACT_FIELD_FOUND_IN_XML.")
    ...
    if root.find("{http://pkp.sfu.ca}id[@type='internal']") is not None:
        ...
        logging.warning(
            "No article ID found in XML. Replacing with -999999999.")
        data_dict["artid"] = PlainInt(-999999999)

    # Parse volume number (take XML volume if present and no arg given; arg 
    # volume always overwrites XML volume)
    ...
    if publication_data.find(".//{http://pkp.sfu.ca}keywords") is not None:
        ...
        data_dict["keywords"] = PlainList(
            create_keyword_list_4yaml(keyword_node))
    ...
    if year is not None:
        ...
        if re.match(r"\d{4}", year):
            ...
            # If the date is valid put appears to be strange as year of 
            # publication, send warning
            if int(year) < 2018 or int(year) > 2100:
                logging.warning(
                    f"""The passed year {year} is valid but appears to be 
                        strange as date of publication. Please verify if the 
                        year is correct."""
                        )
        else:
            logging.warning(
                f"""No year was added to yaml. The year argument {year} cannot 
                    be parsed. Is it really a year with four digits?"""
                    )
    ...
    for author in authors_node:
        ...
        if orcid_dict is not None:
            for k, v in orcid_dict.items():
                if k.lower() in family_name.lower():
                    author_dict["orcid"] = v
        # add orcid from OJS XML (if present) for entries with no explicitly set 
        # ORCID via CLI arg
        ...
        if affiliations is not None:
            for idx, aff in enumerate(author.findall(".//{http://pkp.sfu.ca}affiliation")):
                ...
                if aff.text is not None:
                    ...
                    aff_list = [aff.strip()
                                for aff in aff_list if aff.strip() != ""]
                    for aff_ in aff_list:
                        author_dict["affiliation"].append(
                            {"organization": aff_})

        # create author hdf string and short name

        # first we need to create two abbreviated versions of name (part. if 
        # given name has multiple parts)
        given_name_parsed_light: str = parse_given_name(
            given_name, abbr_style="light")
        given_name_parsed_full: str = parse_given_name(
            given_name, abbr_style="full")

        # layout for "middle" author (in case authors > 2)
        if auth_index > 0 and auth_index < len(authors_node)-1:
            data_dict["name-hdr"] += ", " + \
                SingleQuotedString(given_name_parsed_light + " " + family_name)
            data_dict["author-meta"] += ", " + \
                SingleQuotedString(given_name_parsed_light + " " + family_name)
            data_dict["name-short"] += ", " + \
                SingleQuotedString(family_name + ", " + given_name_parsed_full)
            data_dict["name-long"] += ", " + \
                SingleQuotedString(given_name + " " + family_name)
        # layout for "last" author (in case authors > 2)
        elif auth_index > 0 and auth_index == len(authors_node)-1 and auth_index != 1:
            data_dict["name-hdr"] += ", and " + \
                SingleQuotedString(given_name_parsed_light + " " + family_name)
            data_dict["author-meta"] += ", " + \
                SingleQuotedString(given_name_parsed_light + " " + family_name)
            data_dict["name-short"] += ", & " + \
                SingleQuotedString(family_name + ", " + given_name_parsed_full)
            data_dict["name-long"] += ", and " + \
                SingleQuotedString(given_name + " " + family_name)
        ...
        # layout for first or only one author
            ...
            data_dict["name-hdr"] = SingleQuotedString(
                given_name_parsed_light + " " + family_name)
            data_dict["author-meta"] = SingleQuotedString(
                given_name_parsed_light + " " + family_name)
            data_dict["name-short"] = SingleQuotedString(
                family_name + ", " + given_name_parsed_full)
            data_dict["name-long"] = SingleQuotedString(
    ...
    with open("yaml_output/metadata.yaml", "w", encoding="utf-8") as f:
        yaml.dump(data_dict, f, allow_unicode=True, default_flow_style=False,
                  explicit_start=True, explicit_end=True, default_style="")
